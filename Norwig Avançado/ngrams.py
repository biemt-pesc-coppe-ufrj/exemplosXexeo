...
Original file is located at
    https://colab.research.google.com/drive/1R-Ggs-FTqwNlnacmn3aYegM1La4N4O-y
"""
...
import re, random,  operator, heapq
...
################ Memoization is to save a result previously calculated
def memo(f):
    "Memoize function f."
    table = {}
    def fmemo(*args):
...
def combine(Pfirst, first, prem_rem_tuple):  # fixed for Python 3
  "Combine first and rem results into one (probability, words) pair."
  Prem, rem = prem_rem_tuple
...
# Calcula a probabilidade de uma edição
def Pedit(edit):
...
def edits(word, d=2):
    ...
    def editsR(hd, tl, d, edits):
        ...
        C = hd+tl
        if C in Pw: #is C a word in my examples?
            e = '+'.join(edits)
            if C not in results:
                results[C] = e
            else:
        ...
        #todas as maneiras de expandir
        #a primeira parte da palavra
        #que aparecem na lista de prefixos
        extensions = [hd+c for c in alphabet if hd+c in PREFIXES]
        # o p vai formar a regra de edição
        # letra ou nada
        p = (hd[-1] if hd else '<') ## previous character

        # gerando as inserções
        # todos os hs nas extensões
        # somando a edição
        # roda recursivamente
        # diminuido 1 da edição máxima
        ...
        # para fazer as deleções
        # basta rodar recursivamente com menos 1
        # caracter no tl
        ## Deletion
        editsR(hd, tl[1:], d-1, ed(p, p+tl[0]))
        # se o inicio válido casa, manda fazer com
        # a mesma distância, senão com -1
        for h in extensions:
            if h[-1] == tl[0]: ## Match
